...
public class StartCommand extends BotCommand {
    ...
    public void execute(AbsSender absSender, User user, Chat chat, String[] strings) {
        ...
        if (base.isUserId(user.getId())) {
            sb.append("Hi again! ").append(EmojiParser.parseToUnicode(" :heart_eyes: \n"))
                    .append("I really thought you wouldn't come back... \n")
                    .append("If you forgot that i can look '/help'");
        } else {
            sb.append("Hi ").append(user.getUserName()).append("! Welcome to My WeatherBot!")
                    .append(EmojiParser.parseToUnicode(" :heart_eyes: \n"))
                    .append("To view my capabilities click '/help");
            base.createNewMessage(user.getId());
        }
        message.setText(sb.toString());
        base.updateCurrentMessage(user.getId(), "/start");
        try {
